...
const ProfileModal = ({setShowEdit}) => {
  ...
  return (
    <div className='fixed top-0 bottom-0 left-0 right-0 z-110 h-screen overflow-y-scroll bg-black/70 backdrop-blur'>
        <div className='max-w-2xl sm:py-6 mx-auto'>
            <div className='bg-slate-900 rounded-lg shadow-xl p-6 border border-slate-800 text-slate-100'>
                <h1 className='text-2xl font-bold text-slate-50 mb-6'>Edit Profile</h1>
                <form className='space-y-4' onSubmit={e => toast.promise(
                    handleSaveProfile(e), {loading: 'Saving...'}
                     )}>
                    {/* profile picture  */}
                    <div className='flex flex-col items-start gap-3'>
                        <label htmlFor="profile_picture" className='block text-sm font-medium text-slate-200 mb-1'>
                            Profile Picture 
                    ...
                    {/*Cover Photo */}
                    <div className='flex flex-col items-start gap-3'>
                        <label htmlFor="cover_photo" className='block text-sm font-medium text-slate-200 mb-1'>
                            Cover Photo 
                    ...
                    <div>
                        ...
                        <input type="text" className='w-full p-3 border border-slate-700 rounded-lg bg-slate-950/60 text-slate-100' placeholder='Please enter your full name' onChange={(e)=> setEditForm({...editForm, full_name: e.target.value})} value={editForm.full_name}/>
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-slate-200 mb-1">Username</label>
                        <input type="text" className='w-full p-3 border border-slate-700 rounded-lg bg-slate-950/60 text-slate-100' placeholder='Please enter a username' onChange={(e)=> setEditForm({...editForm, username: e.target.value})} value={editForm.username}/>
                    </div>
                    
                    <div>
                        <label className="block text-sm font-medium text-slate-200 mb-1">Bio</label>
                        <textarea rows={3} className='w-full p-3 border border-slate-700 rounded-lg bg-slate-950/60 text-slate-100' placeholder='Please enter a short bio' onChange={(e)=> setEditForm({...editForm, bio: e.target.value})} value={editForm.bio}/>
                    </div>
                    
                    <div>
                        <label className="block text-sm font-medium text-slate-200 mb-1">Location</label>
                        <input type="text" className='w-full p-3 border border-slate-700 rounded-lg bg-slate-950/60 text-slate-100' placeholder='Please enter your location' onChange={(e)=> setEditForm({...editForm, location: e.target.value})} value={editForm.location}/>
                    </div>

                    <div className='flex justify-end space-x-3 pt-6'>
                        <button onClick={()=> setShowEdit(false)} type='button' className='px-4 py-2 border border-slate-700 rounded-lg text-slate-200 hover:bg-slate-800 transition-colors cursor-pointer'>Cancel</button>
