...
const UserCard = ({user}) => {
  ...
  return (
    <div key={user._id} className='p-4 pt-6 flex flex-col justify-between w-72 shadow border border-slate-800 rounded-md bg-slate-900/90 text-slate-100'>
        <div className='text-center'>
            <img src={user.profile_picture} alt="" className='rounded-full w-16 shadow-md mx-auto'/>
            <p className='mt-4 font-semibold'>{user.full_name}</p>
            {user.username && <p className='text-slate-400 font-light'>@{user.username}</p>}
            {user.bio && <p className='text-slate-300 mt-2 text-center text-sm px-4'>{user.bio}</p>}
        </div>

        <div className='flex items-center justify-center gap-2 mt-4 text-xs text-slate-400'>
            <div className='flex items-center gap-1 border border-slate-700 rounded-full px-3 py-1'>
